<?php
/**
 * Plugin Name: Categorized Popular Posts Widget
 */
add_action( 'widgets_init', 'trada5p_popcatload_widgets' );
function trada5p_popcatload_widgets() {
	register_widget( 'trada5p_popcatwidget' );
}
class trada5p_popcatwidget extends WP_Widget {
	/**
	 * Widget setup.
	 */
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'trada5p_popcatwidget', 'description' => __('A widget that displays a list of popular posts within a time period of your choice.', 'mvp-text') );
		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'trada5p_popcatwidget' );
		/* Create the widget. */
		parent::__construct( 'trada5p_popcatwidget', __('Trada: Categorized Popular Posts Widget', 'mvp-text'), $widget_ops, $control_ops );
	}
	/**
		...
		?>
			<div class="blog-widget-wrap left relative">
				<ul class="blog-widget-list left relative">
					<?php $popular_days_ago = "$popular_days days ago"; $recent = new WP_Query(array( 'cat' => $categories, 'posts_per_page' => $number, 'orderby' => 'meta_value_num', 'order' => 'DESC', 'meta_key' => 'post_views_count', 'date_query' => array( array( 'after' => $popular_days_ago )) )); while($recent->have_posts()) : $recent->the_post(); ?>
						<li>
